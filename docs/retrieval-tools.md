...
| `lcm_compute` | Execute a question-derived, provider-neutral date/count/sum/difference/order/latest-state operation over exact raw spans or assertion IDs. Fixed-operand calculations return `computed`; open-cardinality operations return `subset_computed` with explicitly unverified historical coverage. Caller `evidence_complete` claims never prove completeness. Invalid grounding, mixed units, or ambiguity fail closed. |
